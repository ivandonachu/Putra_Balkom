<?php
session_start();
include'koneksi.php';
if(!isset($_SESSION["login"])){
	header("Location: logout.php");
	exit;
}
$id=$_COOKIE['id_cookie'];
$result1 = mysqli_query($koneksi, "SELECT * FROM account WHERE id_karyawan = '$id'");
$data1 = mysqli_fetch_array($result1);
$id1 = $data1['id_karyawan'];
$jabatan_valid = $data1['jabatan'];
if ($jabatan_valid == 'Kepala Oprasional') {

}

else{  header("Location: logout.php");
exit;
}
$tanggal_awal = $_POST['tanggal1'];
$tanggal_akhir = $_POST['tanggal2'];
$no_laporan = $_POST['no_laporan'];
$status_saldo = $_POST['status_saldo'];
$rekening = $_POST['nama_rekening'];
$jumlah = $_POST['jumlah'];

$query = mysqli_query($koneksi,"DELETE FROM riwayat_saldo_armada WHERE no_laporan = '$no_laporan'");

if ($status_saldo == 'Masuk') {
			$akses_saldo1 = mysqli_query($koneksi, "SELECT * FROM rekening WHERE nama_rekening = '$rekening'");
			$data_saldo1 = mysqli_fetch_array($akses_saldo1);
			$jumlah_rekening1 = $data_saldo1['jumlah'];
			$jumlah_rekening1_new = $jumlah_rekening1 - $jumlah;

			$query1= mysqli_query($koneksi,"UPDATE rekening SET jumlah = '$jumlah_rekening1_new' WHERE nama_rekening = '$rekening' ");

		
			echo "<script> window.location='../view/VSaldoBaru?tanggal1=$tanggal_awal&tanggal2=$tanggal_akhir';</script>";exit;
			
}
elseif ($status_saldo == 'Keluar') {
			$akses_saldo1 = mysqli_query($koneksi, "SELECT * FROM rekening WHERE nama_rekening = '$rekening'");
			$data_saldo1 = mysqli_fetch_array($akses_saldo1);
			$jumlah_rekening1 = $data_saldo1['jumlah'];
			$jumlah_rekening1_new = $jumlah_rekening1 + $jumlah;

			$query1= mysqli_query($koneksi,"UPDATE rekening SET jumlah = '$jumlah_rekening1_new' WHERE nama_rekening = '$rekening' ");

			
				echo "<script> window.location='../view/VSaldoBaru?tanggal1=$tanggal_awal&tanggal2=$tanggal_akhir';</script>";exit;
			
}