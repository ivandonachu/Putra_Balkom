...
$table = mysqli_query($koneksi, "SELECT * FROM riwayat_perpindahan_saldo a INNER JOIN rekening b ON a.no_rekening=b.no_rekening");
$table2 = mysqli_query($koneksi, "SELECT * FROM rekening");
 ...
 <head>
  ...
  <title>Perpindahan Saldo Toko</title>
...
<div id="content-wrapper" class="d-flex flex-column">
  ...
  <div id="content">
    ...
    <nav class="navbar navbar-expand navbar-light  topbar mb-4 static-top shadow" style="background-color:#2C7873;">
      <?php echo "<a href='VPerpindahanSaldo'><h5 class='text-center sm' style='color:white; margin-top: 8px; '>Perpindahan Saldo Perusahaan</h5></a>"; ?>
...
<div>   
  ...
  <div class="row">
   ...
   <div class="col-md-2">
    <!-- Button Pindah Baja -->
    <div align="right">
      <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#input"> <i class="fas fa-plus-square mr-2"></i> Perpindahan Saldo </button> <br> <br>
    </div>
    <!-- Form Modal  -->
     ...
     <div class="modal-dialog modal-lg" role ="document">
       <div class="modal-content"> 
        <div class="modal-header">
          <h5 class="modal-title"> Form Perpindahan Saldo </h5>
        ...
        <div class="modal-body" align="left">
          <?php  echo "<form action='../proses/proses_perpindahan_saldo' enctype='multipart/form-data' method='POST'>";  ?>
        ...
        <div class="row">
          ...
          <div class="col-md-6">
           <label>Saldo Yang Dipindah</label>
          <select id="saldo_dipindah" name="saldo_dipindah" class="form-control">
            <option>Kas di Tangan</option>
            <option>Kas Armada</option>
            <option>Bank BRI CBM</option>
            <option>Bank BRI Toko</option>
            <option>Bank BRI Jual Baja</option>
            <option>Bank Mandiri</option>
          </select>
        </div>

        <div class="col-md-6">
          <label>Saldo Tujuan</label>
          <select id="saldo_tujuan" name="saldo_tujuan" class="form-control">
            <option>Kas di Tangan</option>
            <option>Kas Armada</option>
            <option>Bank BRI CBM</option>
            <option>Bank BRI Toko</option>
...
<table id="example" class="table-sm table-striped table-bordered dt-responsive nowrap" style="width:100%; ">
  ...
  <tbody>
    ...
    <?php while($data = mysqli_fetch_array($table)){
      $no_pindah = $data['no_pindah'];
      $tanggal =$data['tanggal'];
      $id_karyawan = $data['id_karyawan'];
      $no_rekening = $data['no_rekening'];
      $saldo_tujuan = $data['saldo_tujuan'];
      $jumlah = $data['jumlah_pindah'];
      $file_bukti = $data['file_bukti'];
      $keterangan = $data['keterangan'];

      $result = mysqli_query($koneksi, "SELECT * FROM rekening WHERE no_rekening = '$no_rekening' ");
      $data_rekening = mysqli_fetch_array($result);
      $nama_rekening1 = $data_rekening['nama_rekening'];

      $result2 = mysqli_query($koneksi, "SELECT * FROM rekening WHERE no_rekening = '$saldo_tujuan' ");
      $data_rekening2 = mysqli_fetch_array($result2);
      $nama_rekening2 = $data_rekening2['nama_rekening'];

      echo "<tr>
      <td style='font-size: 14px'>$no_pindah</td>
      <td style='font-size: 14px'>$tanggal</td>
      <td style='font-size: 14px'>$id_karyawan</td>
      <td style='font-size: 14px'>$nama_rekening1</td>
      <td style='font-size: 14px'>$nama_rekening2</td>
      ...
      <button href="#" type="submit" class="fas fa-trash-alt bg-danger mr-2 rounded" data-toggle="modal" data-target="#PopUpHapus<?php echo $data['no_pindah']; ?>" data-toggle='tooltip' title='Hapus Transaksi'></button>

      <div class="modal fade" id="PopUpHapus<?php echo $data['no_pindah']; ?>" role="dialog" arialabelledby="modalLabel" aria-hidden="true">
       <div class="modal-dialog" role ="document">
         <div class="modal-content"> 
          ...
          <div class="modal-body">
            <form action="../proses/hapus_perpindahan_saldo" method="POST">
              <input type="hidden" name="no_pindah" value="<?php echo $no_pindah; ?>">
              <input type="hidden" name="nama_rekening1" value="<?php echo $nama_rekening1; ?>">
              <input type="hidden" name="nama_rekening2" value="<?php echo $nama_rekening2;?>">
              <input type="hidden" name="jumlah_pindah" value="<?php echo $jumlah;?>">
