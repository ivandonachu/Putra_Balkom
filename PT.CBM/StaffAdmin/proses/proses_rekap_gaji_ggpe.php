...
while($data2 = mysqli_fetch_array($table)){
    ...
    $insentif = $data2['insentif'];
        ...
        $total_gaji_diterima = $gaji_pokok + $tunjangan_jabatan + $tunjangan_akomodasi + $tunjangan_operasional + $uang_makan_bulan + $fee_kehadiran + $lembur + $bonus + $insentif -  ($bpjs_ketenagakerjaan + $angsuran_bon_bulanan + $denda_absen + $absen_terlambat)  ;
...
$query = mysqli_query($koneksi,"INSERT INTO rekap_gaji_ggpe VALUES('','$tanggal','$nama_karyawan','$jabatan','$gaji_pokok','$tunjangan_jabatan','$tunjangan_akomodasi','$tunjangan_operasional','$bpjs_kesehatan','$bpjs_ketenagakerjaan','$uang_makan_bulan','$fee_kehadiran','$lembur'
                                                                    ,'$absen_terlambat','$denda_absen','$angsuran_bon_bulanan','$bonus','$insentif','$total_gaji','$total_gaji_diterima','$keterangan')");

}

if ($query != "") {
	echo "<script>alert('Proses Rekap Gaji Berhasil :)'); window.location='../view/VRekapGajiGGPE';</script>";exit;
