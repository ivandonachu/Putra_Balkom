<?php
session_start();
include'koneksi.php';
if(!isset($_SESSION["login"])){
	header("Location: logout.php");
	exit;
}
$id=$_COOKIE['id_cookie'];
$result1 = mysqli_query($koneksi, "SELECT * FROM account WHERE id_karyawan = '$id'");
$data1 = mysqli_fetch_array($result1);
$id1 = $data1['id_karyawan'];
$jabatan_valid = $data1['jabatan'];
if ($jabatan_valid == 'Staff Admin') {

}

else{  header("Location: logout.php");
exit;
}



    $nama_driver =$_POST['nama_driver'];
    $jabatan = $_POST['jabatan'];
    $rit_nje = $_POST['rit_nje'];
    $upah_nje = 100000 * $rit_nje;
    $rit_gas_palembang = $_POST['rit_gas_palembang'];
    $upah_gas_palembang = 150000 * $rit_gas_palembang;
    $tunjangan_asuransi = $_POST['tunjangan_asuransi'];
    $lain_lain = $_POST['lain_lain'];
    $angsuran_bon_bulanan = $_POST['angsuran_bon_bulanan'];
    $total_gaji = $upah_nje + $upah_gas_palembang + $tunjangan_asuransi;
    $total_gaji_diterima =  $upah_nje + $upah_gas_palembang ;
    $keterangan = $_POST['keterangan'];

$query = mysqli_query($koneksi,"INSERT INTO list_gaji_driver_cbm VALUES('','$nama_driver','$jabatan','$rit_nje','$upah_nje','$rit_gas_palembang','$upah_gas_palembang','$tunjangan_asuransi','$lain_lain','$angsuran_bon_bulanan','$total_gaji',
                                                                        '$total_gaji_diterima','$keterangan')");



if ($query != "") {
	echo "<script>alert('Proses Rekap Gaji Berhasil :)'); window.location='../view/VListGajiDriverCBM';</script>";exit;

}

?>