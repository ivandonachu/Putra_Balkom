...
$result = mysqli_query($koneksi, "SELECT * FROM pembelian_sl WHERE no_do = '$no_do' ");
 if(mysqli_num_rows($result) == 1 ){
  	echo "<script>alert('DO sudah tercatat :)'); window.location='../view/VPenebusanL?tanggal1=$tanggal_awal&tanggal2=$tanggal_akhir';</script>"; exit;
      }
}
...
if ($nama_file == "") {
	...
	function upload(){
		...
		move_uploaded_file($tmp_name, '../../AdminSemen/file_admin_semen/' . $nama_file_baru   );
	...
	$query = mysqli_query($koneksi,"INSERT INTO pembelian_sl VALUES('','$tanggal','$no_do','$tipe_semen','$tujuan','$nama_kota','$tarif_pranko','$material','$qty','$harga','$jumlah','$driver','$no_polisi','$tipe_bayar', '$tempo','$keterangan','$file')");
			...
			if ($query != "") {
			echo "<script>alert('Data Proses Berhasil :)'); window.location='../view/VPenebusanL?tanggal1=$tanggal_awal&tanggal2=$tanggal_akhir';</script>";exit;
