...
if ($tanggal_awal == $tanggal_akhir) {
  ...
  $table = mysqli_query($koneksi, "SELECT a.tgl_perbaikan ,a.no, a.no_polisi, a.wilayah_operasi , SUM(rit) AS total_rit , SUM(jt_gps) AS total_jt_gps , SUM(jt_odo) AS total_jt_odo FROM kendaraan a INNER JOIN pengiriman_ub b ON a.no=b.no WHERE tanggal = '$tanggal_awal' GROUP BY a.no_polisi ");

}
else{

  $table = mysqli_query($koneksi, "SELECT a.tgl_perbaikan ,a.no, a.no_polisi, a.wilayah_operasi , SUM(rit) AS total_rit , SUM(jt_gps) AS total_jt_gps , SUM(jt_odo) AS total_jt_odo FROM kendaraan a INNER JOIN pengiriman_ub b ON a.no=b.no WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' GROUP BY a.no_polisi ");
...
<div id="content">
    ...
    <?php echo "<a href=''><h5 class='text-center sm' style='color:white; margin-top: 8px; '>Rincian Ritase Ujung Berung</h5></a>"; ?>
