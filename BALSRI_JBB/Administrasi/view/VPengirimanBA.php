...
if ($tanggal_awal == $tanggal_akhir) {
  ...
  $table = mysqli_query($koneksi, "SELECT * FROM pengiriman_ba a INNER JOIN driver b ON a.no_driver=b.no_driver INNER JOIN kendaraan c ON c.no=a.no WHERE tanggal = '$tanggal_awal' ORDER BY a.tanggal");

  $table2 = mysqli_query($koneksi, "SELECT SUM(jt_gps) AS total_jt_gps, SUM(jt_odo) AS total_jt_odo , SUM(dexlite) AS total_dex, SUM(um) AS uang_makan, SUM(ug) AS uang_gaji, SUM(uj) AS uang_jalan FROM pengiriman_ba WHERE tanggal = '$tanggal_awal'");
  ...
  $table3 = mysqli_query($koneksi,"SELECT * FROM pengiriman_ba WHERE tanggal = '$tanggal_awal' AND jns_trans = 'Lost' ");
  ...
  $table4 = mysqli_query($koneksi,"SELECT * FROM pengiriman_ba WHERE tanggal = '$tanggal_awal' AND jns_trans = 'Surplus' ");
  ...
  $table = mysqli_query($koneksi, "SELECT * FROM pengiriman_ba a INNER JOIN driver b ON a.no_driver=b.no_driver INNER JOIN kendaraan c ON c.no=a.no WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' ORDER BY a.tanggal");

  $table2 = mysqli_query($koneksi, "SELECT SUM(jt_gps) AS total_jt_gps, SUM(jt_odo) AS total_jt_odo , SUM(dexlite) AS total_dex, SUM(um) AS uang_makan, SUM(ug) AS uang_gaji, SUM(uj) AS uang_jalan FROM pengiriman_ba WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'");
  $data2 = mysqli_fetch_array($table2);
  $jml_jt_gps= $data2['total_jt_gps'];
  $jml_jt_odo= $data2['total_jt_odo'];
  $jml_dex= $data2['total_dex'];
  ...
  $table3 = mysqli_query($koneksi,"SELECT SUM(jml_trans) AS lost FROM pengiriman_ba WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND jns_trans = 'Lost' ");
  $data3 = mysqli_fetch_array($table3);
  $total_lost = $data3['lost'];
  if (!isset($data3['lost'])) {
    $total_lost = 0;
}
  $table4 = mysqli_query($koneksi,"SELECT SUM(jml_trans) AS surplus FROM pengiriman_ba WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND jns_trans = 'Surplus' ");
  $data4 = mysqli_fetch_array($table4);
...
<div id="content">
    ...
    <?php echo "<a href=''><h5 class='text-center sm' style='color:white; margin-top: 8px; '>Pengiriman Balongan</h5></a>"; ?>
...
<div>   
  ...
  <div class="pinggir1" style="margin-right: 20px; margin-left: 20px;">
    ...
    <?php  echo "<form  method='POST' action='VPengirimanBA' style='margin-bottom: 15px;'>" ?>
    <div>
     ...
     <div class="modal-dialog modal-lg" role ="document">
       <div class="modal-content"> 
        ...
        <div class="modal-body" align="left">
          <?php  echo "<form action='../proses/proses_pengiriman_BA?tanggal1=$tanggal_awal&tanggal2=$tanggal_akhir' enctype='multipart/form-data' method='POST'>";  ?>
         ...
         <div class="row">
          ...
          <div class="col-md-6">
           ...
           <select id="amt" name="amt" class="form-control ">
            <?php
            include 'koneksi.php';
            $result = mysqli_query($koneksi, "SELECT * FROM driver WHERE alamat = 'Balongan'");   
        ...
        <div class="col-md-6">
          ...
          <select id="mt" name="mt" class="form-control">
            <?php
            include 'koneksi.php';
            $result = mysqli_query($koneksi, "SELECT * FROM kendaraan WHERE wilayah_operasi = 'Balongan'");   
        ...
        <div class="modal-dialog modal-lg" role ="document">
          <div class="modal-content"> 
            ...
            <div class="modal-body">
              <form action="../proses/edit_pengiriman_BA" enctype="multipart/form-data" method="POST">
...
<div class="modal fade" id="PopUpHapus<?php echo $data['no_laporan']; ?>" role="dialog" arialabelledby="modalLabel" aria-hidden="true">
 <div class="modal-dialog" role ="document">
   <div class="modal-content"> 
    ...
    <div class="modal-body">
      <form action="../proses/hapus_pengiriman_BA" method="POST">
        <input type="hidden" name="no_laporan" value="<?php echo $no_laporan;?>">
        <input type="hidden" name="tanggal1" value="<?php echo $tanggal_awal; ?>">
        <input type="hidden" name="tanggal2" value="<?php echo $tanggal_akhir;?>">
        <div class="form-group">
