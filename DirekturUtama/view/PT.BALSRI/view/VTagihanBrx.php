...
if ($tanggal_awal == $tanggal_akhir) {
  $table = mysqli_query($koneksibalsri, "SELECT * FROM tagihan_brx a INNER JOIN master_tarif_brx b ON a.no=b.no WHERE tanggal = '$tanggal_awal'");

  $table2 = mysqli_query($koneksibalsri, "SELECT SUM(total) AS total_tagihan, SUM(jt) AS total_jt, SUM(rit) AS total_rit  FROM tagihan_brx a INNER JOIN master_tarif_brx b ON a.no=b.no  WHERE tanggal = '$tanggal_awal'");
  ...
  $table = mysqli_query($koneksibalsri, "SELECT * FROM tagihan_brx a INNER JOIN master_tarif_brx b ON a.no=b.no WHERE tanggal  BETWEEN '$tanggal_awal' AND '$tanggal_akhir'  ORDER BY a.tanggal");

    $table2 = mysqli_query($koneksibalsri, "SELECT SUM(total) AS total_tagihan, SUM(jt) AS total_jt, SUM(rit) AS total_rit  FROM tagihan_brx a INNER JOIN master_tarif_brx b ON a.no=b.no WHERE tanggal  BETWEEN '$tanggal_awal' AND '$tanggal_akhir'");
  $data2 = mysqli_fetch_array($table2);
  $total_tagihan= $data2['total_tagihan'];
  $total_jt= $data2['total_jt'];
  $total_rit= $data2['total_rit'];
...
<div id="content-wrapper" class="d-flex flex-column">
  ...
  <div id="content">
    ...
    <nav class="navbar navbar-expand navbar-light  topbar mb-4 static-top shadow" style="background-color:#2C7873;">
      <?php echo "<a href=''><h5 class='text-center sm' style='color:white; margin-top: 8px;  '>Tagihan Angkutan Pertashop Baturaja</h5></a>"; ?>
...
<div>   
  ...
  <div class="pinggir1" style="margin-right: 20px; margin-left: 20px;">
    ...
    <?php  echo "<form  method='POST' action='VTagihanBrx' style='margin-bottom: 15px;'>" ?>
    <div>
