...
if ($tanggal_awal == $tanggal_akhir) {
    // Tagihan
    $table = mysqli_query($koneksibalsri, "SELECT SUM(total) AS total_tagihan, SUM(jt) AS total_jt, SUM(rit) AS total_rit  FROM tagihan a INNER JOIN master_tarif b ON a.delivery_point=b.delivery_point  WHERE tanggal = '$tanggal_awal'  AND no_polisi ='$no_polisilr'");
    ...
    //pengiriman
    $table2 = mysqli_query($koneksibalsri, "SELECT SUM(um) AS uang_makan FROM pengiriman_spbu a INNER JOIN kendaraan b ON a.no=b.no WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND b.no_polisi = '$no_polisilr'");
    $data2 = mysqli_fetch_array($table2);
    ...
    $total_dexlite = 0;
    $total_bbm = 0;
    $table222 = mysqli_query($koneksibalsri, "SELECT jt_gps, uj, dexlite FROM pengiriman_spbu a INNER JOIN kendaraan b ON a.no=b.no WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND b.no_polisi = '$no_polisilr'");
    ...
    //Gaji dRIVER
    $table9 = mysqli_query($koneksibalsri, "SELECT SUM(a.ug) AS jumlah_gaji FROM pengiriman_spbu a INNER JOIN kendaraan b ON a.no=b.no WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND b.no_polisi = '$no_polisilr'");
...
// PPH 23 2%
$jumlah_pph_23 = ((($total_tagihan + $total_tagihan_spbu) * 2) / 100);

$total_laba_pph23 = ($total_tagihan + $total_tagihan_spbu) - $jumlah_pph_23;
