...
if ($tanggal_awal == $tanggal_akhir) {
  // Tagihan
  $table = mysqli_query($koneksibalsri, "SELECT SUM(total) AS total_tagihan, SUM(jt) AS total_jt, SUM(rit) AS total_rit  FROM tagihan_p a INNER JOIN master_tarif_p b ON a.no=b.no  WHERE tanggal = '$tanggal_awal'");
  ...
  //pengeluran Pul Biaya Kantor
   ...
   $jml_perbaikan = $data7['jumlah_perbaikan'];
    ...
    // Tagihan
  $table = mysqli_query($koneksibalsri, "SELECT SUM(total) AS total_tagihan, SUM(jt) AS total_jt, SUM(rit) AS total_rit  FROM tagihan_p a INNER JOIN master_tarif_p b ON a.no=b.no  WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'");
  $data = mysqli_fetch_array($table);
  $total_tagihan= $data['total_tagihan'];
  //pengiriman
  $table2 = mysqli_query($koneksibalsri, "SELECT SUM(dexlite) AS total_dex, SUM(um) AS uang_makan FROM pengiriman_p WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'");
  $data2 = mysqli_fetch_array($table2);
  $jml_dex= $data2['total_dex'];
  $total_um= $data2['uang_makan'];
  $total_dexlite = $jml_dex * 9700;
  
       $table22 = mysqli_query($koneksibalsri, "SELECT *  FROM pengiriman WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' ");
  
  $total_ug =0;
  while($data22 = mysqli_fetch_array($table22)){
    $ug = $data22['ug'];
   
    if($ug  > 0){
        $total_ug = $total_ug + $ug;
    }
    else{
 ...
 //Gaji karyawan
   ...
   $gaji_driver = $data9['jumlah_gaji'];
    ...
    $total_biaya_usaha_final = $total_dexlite + $jml_biaya_kantor + $jml_listrik + $jml_sewa + $jml_atk + $jml_perbaikan+ $total_ug + $total_um + $total_gaji_karaywan;
?>
