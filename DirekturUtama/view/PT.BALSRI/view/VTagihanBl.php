...
if ($tanggal_awal == $tanggal_akhir) {
  $table = mysqli_query($koneksibalsri, "SELECT * FROM tagihan_bl a INNER JOIN master_tarif_bl b ON a.delivery_point=b.delivery_point WHERE tanggal = '$tanggal_awal'");

  $table2 = mysqli_query($koneksibalsri, "SELECT SUM(total) AS total_tagihan, SUM(jt) AS total_jt, SUM(rit) AS total_rit  FROM tagihan_bl a INNER JOIN master_tarif_bl b ON a.delivery_point=b.delivery_point  WHERE tanggal = '$tanggal_awal'");
  $data2 = mysqli_fetch_array($table2);
  $total_tagihan = $data2['total_tagihan'];
  $total_jt = $data2['total_jt'];
  $total_rit = $data2['total_rit'];
} else {
  $table = mysqli_query($koneksibalsri, "SELECT * FROM tagihan_bl a INNER JOIN master_tarif_bl b ON a.delivery_point=b.delivery_point WHERE tanggal  BETWEEN '$tanggal_awal' AND '$tanggal_akhir'  ORDER BY a.tanggal");

  $table2 = mysqli_query($koneksibalsri, "SELECT SUM(total) AS total_tagihan, SUM(jt) AS total_jt, SUM(rit) AS total_rit  FROM tagihan_bl a INNER JOIN master_tarif_bl b ON a.delivery_point=b.delivery_point WHERE tanggal  BETWEEN '$tanggal_awal' AND '$tanggal_akhir'");
  $data2 = mysqli_fetch_array($table2);
  $total_tagihan = $data2['total_tagihan'];
  $total_jt = $data2['total_jt'];
  $total_rit = $data2['total_rit'];
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <nav class="navbar navbar-expand navbar-light  topbar mb-4 static-top shadow" style="background-color:#2C7873;">
          <?php echo "<a href='VTagihanBl?tanggal1=$tanggal_awal&tanggal2=$tanggal_akhir'><h5 class='text-center sm' style='color:white; margin-top: 8px;  '>Tagihan Angkutan Pertashop Belitung</h5></a>"; ?>
