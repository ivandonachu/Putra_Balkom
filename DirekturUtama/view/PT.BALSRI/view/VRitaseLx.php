...
if ($tanggal_awal == $tanggal_akhir) {
  ...
  $table = mysqli_query($koneksilatex, "SELECT a.tgl_perbaikan ,a.no, a.no_polisi , SUM(rit) AS total_rit , SUM(jt_gps) AS total_jt_gps , SUM(jt_odo) AS total_jt_odo FROM kendaraan a INNER JOIN pengiriman b ON a.no=b.no WHERE tanggal = '$tanggal_awal' GROUP BY a.no_polisi ");

}
else{

  $table = mysqli_query($koneksilatex, "SELECT a.tgl_perbaikan ,a.no, a.no_polisi , SUM(rit) AS total_rit , SUM(jt_gps) AS total_jt_gps , SUM(jt_odo) AS total_jt_odo FROM kendaraan a INNER JOIN pengiriman b ON a.no=b.no_kendaraan WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' GROUP BY a.no_polisi ");

}

if ($tanggal_awal == $tanggal_akhir) {

  $table2 = mysqli_query($koneksilatex, "SELECT  a.nama_driver_1 , SUM(rit) AS total_rit , SUM(jt_gps) AS total_jt_gps , SUM(jt_odo) AS total_jt_odo FROM driver_1 a INNER JOIN pengiriman b ON a.no_driver_1=b.no_driver_1 WHERE tanggal = '$tanggal_awal' GROUP BY a.nama_driver_1 ");

}
else{

  $table2 = mysqli_query($koneksilatex, "SELECT   a.nama_driver_1 , SUM(rit) AS total_rit , SUM(jt_gps) AS total_jt_gps , SUM(jt_odo) AS total_jt_odo FROM driver_1 a INNER JOIN pengiriman b ON a.no_driver_1=b.no_driver_1 WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' GROUP BY a.nama_driver_1 ");
...
<head>
  ...
  <title>Riwayat Ritase Latex</title>
...
<div id="content-wrapper" class="d-flex flex-column">
  ...
  <div id="content">
    ...
    <nav class="navbar navbar-expand navbar-light  topbar mb-4 static-top shadow" style="background-color:#2C7873;">
      <?php echo "<a href='VRitase?tanggal1=$tanggal_awal&tanggal2=$tanggal_akhir'><h5 class='text-center sm' style='color:white; margin-top: 8px; '>Riwayat Ritase Latex</h5></a>"; ?>
...
<div>   
  ...
  <div class="pinggir1" style="margin-right: 20px; margin-left: 20px;">
    ...
    <?php  echo "<form  method='POST' action='VRitaseLx' style='margin-bottom: 15px;'>" ?>
    <div>
  ...
  <tbody>
    ...
    <?php while($data = mysqli_fetch_array($table)){
      ...
      $tgl_perbaikan = $data['tgl_perbaikan'];
        ...
        $total_rit = $total_rit/2;
        $total_jt_gps =$data['total_jt_gps'];
        $total_jt_gps = $total_jt_gps/2;
        $total_jt_odo =$data['total_jt_odo'];
        $total_jt_odo = $total_jt_odo/2;
   
       $table4 = mysqli_query($koneksilatex, "SELECT  SUM(rit) AS total_rit2 , SUM(jt_gps) AS total_jt_gps2 , SUM(jt_odo) AS total_jt_odo2 FROM kendaraan a 
       INNER JOIN pengiriman b ON a.no=b.no_kendaraan WHERE tanggal BETWEEN '$tgl_perbaikan' AND '$tanggal_akhir' AND no_polisi = '$no_polisi' ");
       $data4 = mysqli_fetch_array($table4);

      $total_rit2 = $data4['total_rit2'];
      $total_rit2 = $total_rit2/2;
      $total_jt_gps2 =$data4['total_jt_gps2'];
      $total_jt_gps2 = $total_jt_gps2/2;
      $total_jt_odo2 =$data4['total_jt_odo2'];
      $total_jt_odo2 = $total_jt_odo2/2;
     ...
     ?>
      ...
      <td align = 'center'><a href='VRincianRitKenLx?tanggal1=$tanggal_awal&tanggal2=$tanggal_akhir&no_polisi=$no_polisi'>Rincian</a></td>
      </tr>";
}
...
<table id="example1" class="table-sm table-striped table-bordered dt-responsive nowrap" style="width:100%; ">
  ...
  <tbody>
    ...
    <?php while($data = mysqli_fetch_array($table2)){
      ...
      $total_rit = $total_rit;
      $total_jt_gps =$data['total_jt_gps'];
      $total_jt_gps = $total_jt_gps;
      $total_jt_odo = $data['total_jt_odo'];
      $total_jt_odo = $total_jt_odo;
      ...
      <td style='font-size: 14px' align = 'center'>$total_rit</td>
       <td style='font-size: 14px' align = 'center'>$total_jt_gps</td>
      <td style='font-size: 14px' align = 'center'>$total_jt_odo</td>
      <td  align = 'center'><a href='VRincianRitDriverLx?tanggal1=$tanggal_awal&tanggal2=$tanggal_akhir&nama_driver=$nama_driver'>Rincian</a></td>
      </tr>";
}
?>
