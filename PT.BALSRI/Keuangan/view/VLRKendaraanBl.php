...
if ($tanggal_awal == $tanggal_akhir) {
  // Tagihan
  $table = mysqli_query($koneksi, "SELECT SUM(total) AS total_tagihan, SUM(jt) AS total_jt, SUM(rit) AS total_rit  FROM tagihan_bl a INNER JOIN master_tarif_bl b ON a.delivery_point=b.delivery_point  WHERE tanggal = '$tanggal_awal'  AND no_polisi ='$no_polisilr'");
  ...
  $total_dexlite = $jml_dex * 9700;
   ...
   $jml_perbaikan = $data7['jumlah_perbaikan'];
    ...
    // Tagihan
  $table = mysqli_query($koneksi, "SELECT SUM(total) AS total_tagihan, SUM(jt) AS total_jt, SUM(rit) AS total_rit  FROM tagihan_bl a INNER JOIN master_tarif_bl b ON a.delivery_point=b.delivery_point  WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND mt = '$no_polisilr'");
  $data = mysqli_fetch_array($table);
  $total_tagihan= $data['total_tagihan'];


  //pengiriman
  $table2 = mysqli_query($koneksi, "SELECT SUM(um) AS uang_makan FROM pengiriman_bl a INNER JOIN kendaraan b ON a.no=b.no WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND b.no_polisi = '$no_polisilr'");
  $data2 = mysqli_fetch_array($table2);

  $total_um= $data2['uang_makan'];

  $total_dexlite = 0;
  $table222 = mysqli_query($koneksi, "SELECT jt_gps, uj FROM pengiriman_bl a INNER JOIN kendaraan b ON a.no=b.no WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND b.no_polisi = '$no_polisilr'");
  ...
  $total_kredit= $dataa['total_kredit'];
    ...
    //pengeluran perbaikan
   $table7 = mysqli_query($koneksi, "SELECT SUM(jml_pengeluaran) AS jumlah_perbaikan FROM riwayat_perbaikan_bl WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND no_polisi ='$no_polisilr' ");
   $data7 = mysqli_fetch_array($table7);
   $jml_perbaikan = $data7['jumlah_perbaikan'];
    if (!isset($data7['jumlah_perbaikan'])) {
    $jml_perbaikan = 0;
    ...
    //Gaji dRIVER
    $table9 = mysqli_query($koneksi, "SELECT SUM(a.ug) AS jumlah_gaji FROM pengiriman_bl a INNER JOIN kendaraan b ON a.no=b.no WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND b.no_polisi = '$no_polisilr'");
   ...
   <div class="col-md-12">
      <div class="panel panel-default">
         <div class="panel-heading">
            <h3 class="panel-title" align="Center"><strong>Laba Rugi  <?= $no_polisilr; ?> (Belitung)</strong></h3>
        </div>
