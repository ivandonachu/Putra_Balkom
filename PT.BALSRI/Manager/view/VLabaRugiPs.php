...
elseif($lokasi == 'Sumber Jaya' || $lokasi == 'Nusa Bakti'){
  ...
  while($data2 = mysqli_fetch_array($table2)){
   ...
   $tabley1 = mysqli_query($koneksiperta, "SELECT MIN(no_penjualan) AS penjualan_akhir FROM penjualan a INNER JOIN pertashop b ON b.kode_perta=a.kode_perta WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND nama_barang = 'Pertamax' AND b.lokasi = '$lokasi' ");

   $datay1 = mysqli_fetch_array($tabley1);
   $no_penjualan_akhir_max = $datay1['penjualan_akhir'];

   $tabley12 = mysqli_query($koneksiperta, "SELECT sonding_awal, harga FROM penjualan WHERE no_penjualan = '$no_penjualan_akhir_max' ");
   $datay12 = mysqli_fetch_array($tabley12);
   $stok_akhir_max = $datay12['sonding_awal'];
   $harga_stok_akhir_max = $datay12['harga'];
   $total_uang_stok_max = $stok_akhir_max * $harga_stok_akhir_max;

   //Sisa stok Dexlite
   $tabley2 = mysqli_query($koneksiperta, "SELECT MIN(no_penjualan) AS penjualan_akhir FROM penjualan a INNER JOIN pertashop b ON b.kode_perta=a.kode_perta WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND nama_barang = 'Dexlite' AND b.lokasi = '$lokasi' ");

   $datay2 = mysqli_fetch_array($tabley2);
   $no_penjualan_akhir_dex = $datay2['penjualan_akhir'];

   $tabley22 = mysqli_query($koneksiperta, "SELECT sonding_awal, harga FROM penjualan WHERE no_penjualan = '$no_penjualan_akhir_dex' ");
   $datay22 = mysqli_fetch_array($tabley22);
   $stok_akhir_dex = $datay22['sonding_awal'];
